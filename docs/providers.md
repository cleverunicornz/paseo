...
`yeet-code` is a first-class remote ACP v2 provider for a Yeet Code Gateway installation. It does
not spawn a local agent process and never receives the Gateway's upstream model credentials. Set
the ACP endpoint and Gateway client bearer through the provider override:
  ...
  "agents": {
    "providers": {
      "yeet-code": {
        "enabled": true,
        "params": {
          "url": "http://100.79.9.58:18445/acp"
        },
        "env": {
          "YEET_CODE_GATEWAY_TOKEN": "<gateway-client-bearer>"
        }
      }
    }
  }
...
The provider uses standard ACP v2 over HTTP/SSE, discovers composed models from session
configuration, and supports Gateway-owned session listing, resume/replay, cancellation, images,
and embedded resources. The token authenticates Paseo to the Gateway only.
